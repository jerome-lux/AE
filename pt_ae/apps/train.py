...
from pt_ae.ae.loss import G_GANLoss, D_GANLoss, ZeroCenteredGradientPenalty
...
def train(
    ...
    for epoch in range(epochs):
        ...
        for i, (real_data, _) in enumerate(progress_bar):
            ...
            loss = train_step(model, real_data, real_data, loss_fn, optimizer, lossweights=lossweights)

            # Update loss and accuracy
            loss_meter.update(loss.item(), real_data.size(0))
        ...
        writer.add_scalar("Gen_loss", avg_loss, epoch + 1)
